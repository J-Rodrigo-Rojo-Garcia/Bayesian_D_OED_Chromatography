program Training_Smolyak_Individual
...
real(kind = 8),allocatable,dimension(:,:)::Lag,theta_pr,d0,X,Fk
...
! Design Space parameters
 ...
 call Read_OED_General_Params(filename,u_file + 6,i,N_OED_T)

! Design Space
 allocate(d0(N_OED_T,N_OED)) 
 filename = 'fortran_data/design_space.dat' 	 
 d0 = Read_Design_Space(filename,u_file + 7,N_OED_T,N_OED)
...
! Create character variables
 ...
 filename = 'fortran_data/training_names.dat'
 N_char = len(trim(adjustl(filename)))
...
! Begin the training
 ...
 do j = 1,N_OED_T
 	do i = 1,Nk
 		Fk(i,:) = Forward_Mapping(Nc,Ny,N_tao,Nt_d,Nd,X(i,4),Ind,d_tao,dy,d0(j,1),F,X(i,3),d0(j,2),X(i,1:2),Lag)
 	enddo
