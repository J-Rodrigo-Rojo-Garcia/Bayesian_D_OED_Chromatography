function plots_MSE_piecewise_2(t_dat,yr,ys,titles,names,depth)	
    ...
    leg1{1} = 'True'; 
    for k = 1:N
		leg1{k+1} = join(['Depth = ', int2str(depth)]);	    
    end
    ...
    plot(t_dat,yr(I1),'k-')
    hold on
    plot(t_dat,ys(I1),colors{mod(depth,5)+1})
    hold off	
	...
	plot(t_dat,yr(I2),'k-')
	hold on
	plot(t_dat,ys(I2),colors{mod(depth,5)+1})
	hold off	
	xlabel("t",'Interpreter','tex') % "latex" in Matlab
	ylabel("c",'Interpreter','tex') % "latex" in Matlab
	tit = join([titles,' c2']);	
