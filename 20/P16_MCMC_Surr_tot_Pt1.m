...
% 9) Training objects
filename ='results/surrogated_model_tot/PSI_tot.mat'; 
Z = load(filename); Z = Z.Z;
...
for k = 1:Depth_max
	...
	parfor m = 1:N_OED_1
		...
		% Surrogated function
		dm = d1(m,:);
		FM = @(theta) interp_CC_vect(z,[theta,dm],Nd,1)';

		% Temporal directory
		abs_dir = join([dire,'/',dire_2]);
		
		% Evaluation in the true parameter
		filename = join([abs_dir,'/c_tot_depth_',int2str(k),'_',aux,'.dat']);
		ck = FM(Theta_true');
