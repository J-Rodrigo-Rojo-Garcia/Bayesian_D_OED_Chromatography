...
real(kind = 8),allocatable,dimension(:,:)::Lag,c
...
! Uncertainty and design variables
 allocate(theta(N_th),d(N_OED),c(N_tao,Nc),cI(Nd))
...
! Forward Mapping
 call cpu_time(start)
 call FM_Koren(Nc,Ny,N_tao,Nt_d,Nd,theta(4),Ind,d_tao,dy,d(1),&
 F,theta(3),d(2),theta(1:2),Lag,c,cI)
...
! Save data
 ...
 filename = 'results/forward_mapping_evaluation/c.dat'
 call Write_c(filename,u_file + 6,N_tao,c)
